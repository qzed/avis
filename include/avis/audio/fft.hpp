...
constexpr auto cos(real_t x) -> real_t {
    return std::cos(x);     // TODO: make constexpr
}

template <class real_t>
constexpr auto sin(real_t x) -> real_t {
    return std::sin(x);     // TODO: make constexpr
}
...
constexpr auto hanning_window_table_entry(std::size_t i) -> real_t {
    return 0.5 * (1.0 - math::cos((2.0 * math::pi<real_t> * i) / (N - 1.0)));
}
...
void rfft(real_t* data, real_t* dst) {
    ...
    constexpr static auto scale = 1.0 / math::sqrt(static_cast<real_t>(N));
